...
import { AddClientFormSchema, AddClientFormType, CLIENT_TYPE_ENUM } from "@/lib/zod";
...
export function AddNewClient({ clientType }: { clientType: string }) {
  ...
  const addClient = async (clientData: {
    ...
    const { data: storeResponse, error: storeError } = await supabase.from("clients").insert([clientData]).select();
    if (storeError) {
  ...
  async function onSubmit(values: AddClientFormType) {
    console.log(values);
    setLoading(true);
      const clientResponse = await addClient({ ...values, store_id: selectedStore?.id!});
  ...
  return (
    <Dialog open={showNewStoreDialog} >
      <DialogTrigger asChild>
        <Button className="space-x-2" onClick={() => setShowNewStoreDialog(true)}>
      ...
      <DialogContent className="sm:max-w-[625px]">
        <DialogHeader>
          ...
          <DialogDescription>Enter the below details to add a new {clientType}</DialogDescription>
        </DialogHeader>
        <Form {...form}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input type="text" placeholder={`Enter ${clientType} name...`} {...field} />
                  </FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input type="text" placeholder={`Enter ${clientType} address...`} {...field} />
                  </FormControl>
                  <FormMessage className="text-xs" />
                </FormItem>
              )}
