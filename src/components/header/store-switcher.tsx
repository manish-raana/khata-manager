...
export default function StoreSwitcher({ className }: StoreSwitcherProps) {
  ...
  const { selectedStore, setSelectedStore, storeList, getStoreList } = useStoreList();
  ...
  return (
    <Dialog open={showNewStoreDialog} onOpenChange={setShowNewStoreDialog}>
        ...
        <PopoverTrigger asChild>
          ...
          >
            <Avatar className="mr-2 h-5 w-5">
              <AvatarImage src={`https://avatar.vercel.sh/${selectedStore?.id}.png`} alt={selectedStore?.name} className="grayscale" />
          ...
          <Command>
            <CommandList>
              ...
              {storeList?.map((store: IStore) => (
                ...
                >
                  <Avatar className="mr-2 h-5 w-5">
                    <AvatarImage src={`https://avatar.vercel.sh/${store.id}.png`} alt={store.name} className="grayscale" />
                    <AvatarFallback>SC</AvatarFallback>
                  </Avatar>
                  {store.name}
      ...
      <DialogContent>
        ...
        <AddStore getStoreList={getStoreList} setShowNewStoreDialog={setShowNewStoreDialog} />
        {/* <DialogFooter>
